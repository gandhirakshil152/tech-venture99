...
const services = [
  ...
  },
  {
    ...
    color: "from-[hsl(200,100%,50%)] to-[hsl(220,100%,65%)]",
    shadow: "hsl(200,100%,55%)",
  },
  {
    icon: Megaphone,
  ...
  },
  {
    ...
    color: "from-[hsl(340,80%,55%)] to-[hsl(360,80%,60%)]",
    shadow: "hsl(340,80%,55%)",
  },
  {
    icon: Search,
    ...
    color: "from-[hsl(120,60%,40%)] to-[hsl(150,70%,50%)]",
    ...
    <main className="pt-24 md:pt-28 pb-20 min-h-screen">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="font-accent text-primary text-sm tracking-[0.3em] uppercase mb-3">What We Do</p>
          <h1 className="text-3xl md:text-5xl font-display font-bold gradient-text mb-4">Our Services</h1>
          <p className="text-muted-foreground max-w-lg mx-auto">
            From design to deployment, we offer end-to-end digital solutions to help your business thrive online.
          </p>
        </motion.div>

        {/* Services Grid */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-20 max-w-6xl mx-auto">
          {services.map((service, i) => (
            ...
            >
              <div
                ...
                style={{ background: `radial-gradient(circle at center, ${service.shadow}, transparent 70%)` }}
              />
              <div className="relative z-10">
                <div
                  className={`w-12 h-12 rounded-xl bg-gradient-to-br ${service.color} flex items-center justify-center mb-4 shadow-lg`}
                  style={{ boxShadow: `0 4px 20px ${service.shadow}40` }}
                >
                  <service.icon className="w-6 h-6 text-primary-foreground" />
                </div>
                <h3 className="font-display text-lg text-foreground mb-2 group-hover:neon-text transition-all duration-300">
                  {service.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {service.techs.map((tech) => (
                    <span
                      key={tech}
                      className="text-xs px-2.5 py-1 rounded-full bg-secondary/80 text-muted-foreground border border-border"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        >
          <div className="glass rounded-2xl p-8 md:p-12 neon-border border max-w-2xl mx-auto">
            ...
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 px-8 py-3 rounded-xl font-accent text-sm tracking-wider uppercase bg-gradient-to-r from-primary to-accent text-primary-foreground hover:shadow-[0_0_30px_hsl(183_100%_50%/0.4)] transition-all duration-300 font-semibold"
            >
